...
final class SC_Product_Fields {
    ...
    public static function bulk_edit_save(WC_Product $product): void {
        $map = ['_sc_bulk_izuzeto' => SC_Snapshot::META_EXCL, '_sc_bulk_cjenik' => SC_Snapshot::META_NOCJ];
        $changed = false;
    ...
    public static function save_simple(int $post_id): void {
        ...
        foreach ([SC_Snapshot::META_EXCL, SC_Snapshot::META_NOCJ] as $meta) {
            if (!empty($_POST[$meta]) && $_POST[$meta] === 'yes') {
                update_post_meta($post_id, $meta, 'yes');
            } else {
                delete_post_meta($post_id, $meta);
            }
        }
        self::save_from_request($post_id, (string) wp_unslash($_POST['_sidrena_cijena']), (string) wp_unslash($_POST['_sidrena_cijena_datum'] ?? ''));
    }
    ...
    public static function save_variation(int $variation_id, int $i): void {
        ...
        self::save_from_request($variation_id, (string) wp_unslash($_POST['_sidrena_cijena_var'][$i]), (string) wp_unslash($_POST['_sidrena_cijena_datum_var'][$i] ?? ''));
    }
