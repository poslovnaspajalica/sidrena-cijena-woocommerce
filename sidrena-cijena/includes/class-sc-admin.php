...
final class SC_Admin {
	...
	public static function init(): void {
		...
		add_filter(
			'plugin_action_links_' . plugin_basename( SC_FILE ),
	...
	public static function render(): void {
		$tab  = sanitize_key( $_GET['tab'] ?? 'status' ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- samo odabir kartice za prikaz.
		$tabs = [
			'status'   => 'Status i akcije',
			'postavke' => 'Postavke',
			'cjenici'  => 'Cjenici',
			'uvoz'     => 'Uvoz / izvoz sidrenih cijena',
		];
	...
	private static function tab_status(): void {
		...
		<div class="sc-card">
			...
			<?php
			if ( ! empty( $s['alt_kategorije'] ) ) :
				?>
				(kategorije NN 75/2025: <?php echo esc_html( SC_Settings::format_date( $s['alt_datum'] ) ); ?>)<?php endif; ?>
			· Obveza isticanja i objave cjenika vrijedi od <strong>1. 10. 2026.</strong></p>
			<?php $active = SC_Settings::display_active(); ?>
			<p>Prikaz sidrene cijene na webshopu: 
			<?php
			if ( $s['prikaz_mod'] === 'on' ) {
		...
		<div class="sc-card">
			...
			<?php if ( $last ) : ?>
				...
				(<?php echo (int) $last['rows']; ?> redaka
				<?php
				if ( $last['missing'] ) :
					?>
					, <span class="sc-warn"><?php echo (int) $last['missing']; ?> bez sidrene cijene</span><?php endif; ?>, <?php echo (int) $last['seconds']; ?> s, pokrenuo: <?php echo esc_html( $last['trigger'] ); ?>)
			...
			<?php endif; ?>
			<p>Automatsko generiranje: 
			<?php
			if ( $s['cron_nacin'] === 'wpcron' ) :
				?>
				svaki dan u <strong><?php echo esc_html( $s['cron_vrijeme'] ); ?></strong>
				<?php echo $next ? '(sljedeće: ' . esc_html( wp_date( 'd.m.Y. H:i', $next ) ) . ')' : '<span class="sc-warn">(nije zakazano, spremi postavke)</span>'; ?>
				<?php
				else :
					?>
					<strong>isključeno</strong>, cjenik se generira samo ručno<?php endif; ?>.
				Datoteke se čuvaju <?php echo (int) $s['retencija_dana']; ?> dana. <a href="<?php echo esc_url( self::url( 'postavke' ) ); ?>">Postavke</a></p>
			<?php $lock = SC_Export::lock_info(); ?>
			<?php if ( $lock ) : ?>
		...
		<div class="sc-card">
			...
			<p><code style="user-select:all">
			<?php
			list($h, $m) = explode( ':', $s['cron_vrijeme'] );
			echo esc_html( sprintf( '%d %d * * * curl -s "%s" > /dev/null', (int) $m, (int) $h, $trigger ) );
			?>
			</code></p>
				...
				<table class="form-table">
					...
					<tr><th><label for="label">Tekst oznake</label></th>
						...
						echo wp_kses_post(
							strtr(
								$s['label'],
								[
									'{datum}'     => SC_Settings::format_date( $s['referentni_datum'] ),
									'{datum_iso}' => $s['referentni_datum'],
									'{cijena}'    => wc_price( 19.9 ),
								]
							)
						);
						?>
					...
					<tr><th><label for="izuzete_kategorije">Kategorije izuzete iz isticanja</label></th>
						<td><select name="izuzete_kategorije[]" id="izuzete_kategorije" multiple size="6" style="min-width:300px">
							<?php foreach ( (array) $cats as $c ) : ?>
								<option value="<?php echo (int) $c->term_id; ?>" <?php selected( in_array( (int) $c->term_id, array_map( 'intval', (array) $s['izuzete_kategorije'] ), true ) ); ?>><?php echo esc_html( $c->name ); ?></option>
							<?php endforeach; ?>
						</select>
					...
					<tr><th><label for="alt_kategorije">Kategorije iz Odluke NN 75/2025</label></th>
						<td><select name="alt_kategorije[]" id="alt_kategorije" multiple size="6" style="min-width:300px">
							<?php foreach ( (array) $cats as $c ) : ?>
								<option value="<?php echo (int) $c->term_id; ?>" <?php selected( in_array( (int) $c->term_id, array_map( 'intval', (array) $s['alt_kategorije'] ), true ) ); ?>><?php echo esc_html( $c->name ); ?></option>
							<?php endforeach; ?>
						</select>
				...
				<table class="form-table">
					<tr><th><label>Izvor podatka „marka“</label></th>
						<td><select name="marka_izvor">
							...
							if ( taxonomy_exists( 'product_brand' ) ) :
								?>
								<option value="product_brand" <?php selected( $s['marka_izvor'], 'product_brand' ); ?>>Taksonomija „Brands“ (WooCommerce)</option><?php endif; ?>
							<?php
							...
							<?php endforeach; ?>
							<?php
							if ( str_starts_with( (string) $s['marka_izvor'], 'meta:' ) ) :
								?>
								<option value="<?php echo esc_attr( $s['marka_izvor'] ); ?>" selected>Meta polje: <?php echo esc_html( substr( $s['marka_izvor'], 5 ) ); ?></option><?php endif; ?>
						</select>
					...
					<tr><th><label>Izvor podatka „barkod“</label></th>
						<td><select name="barkod_izvor">
							...
							<?php
							if ( str_starts_with( (string) $s['barkod_izvor'], 'meta:' ) ) :
								?>
								<option value="<?php echo esc_attr( $s['barkod_izvor'] ); ?>" selected>Meta polje: <?php echo esc_html( substr( $s['barkod_izvor'], 5 ) ); ?></option><?php endif; ?>
						</select>
		...
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT p.ID, p.post_type, p.post_title, sku.meta_value AS sku, rp.meta_value AS regular, sc.meta_value AS sidrena, sd.meta_value AS datum, si.meta_value AS izvor
				 ...
				 LEFT JOIN {$wpdb->postmeta} sd ON sd.post_id=p.ID AND sd.meta_key=%s
				 LEFT JOIN {$wpdb->postmeta} si ON si.post_id=p.ID AND si.meta_key=%s
	...
	public static function import_csv(): void {
		...
		// tmp_name generira PHP, nije korisnički unos; is_uploaded_file potvrđuje da je riječ o upravo prenesenoj datoteci.
		...
		$r = self::import_file( $tmp, ! empty( $_POST['samo_objavljeni'] ), ! empty( $_POST['prazno_brise'] ) );
		if ( ! empty( $r['error'] ) ) {
