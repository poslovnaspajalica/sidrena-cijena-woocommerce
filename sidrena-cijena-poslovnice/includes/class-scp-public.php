...
<section class="sec <?php echo $s['id'] === $active ? 'on' : ''; ?>" id="sec-<?php echo esc_attr( $s['id'] ); ?>">
	...
	<table>
		...
		<tbody>
			...
			<tr><td><a href="<?php echo esc_url( $f['url'] ); ?>"><?php echo esc_html( $f['name'] ); ?></a></td><td><?php echo esc_html( strtoupper( $f['ext'] ) ); ?></td><td><?php echo esc_html( isset( $f['vrijedi_za'] ) ? SCP_Settings::format_date( $f['vrijedi_za'] ) : wp_date( 'd.m.Y.', $f['mtime'] ) ); ?></td><td><?php echo esc_html( wp_date( 'd.m.Y. H:i', $f['mtime'] ) ); ?></td><td><?php echo esc_html( size_format( $f['size'] ) ); ?></td></tr>
		<?php endforeach; ?>
		</tbody>
	</table>
</section>
